...
public class Utils {
    ...
    public static float[] sphericalToCartesian(float distanceMeters, float azimuthDegrees, float elevationDegrees) {
        double azimuthRadians = Math.toRadians(azimuthDegrees);
        double elevationRadians = Math.toRadians(elevationDegrees);
        double planarDistance = distanceMeters * Math.cos(elevationRadians);

        float x = (float) (planarDistance * Math.sin(azimuthRadians));
        float y = (float) (distanceMeters * Math.sin(elevationRadians));
        float z = (float) (planarDistance * Math.cos(azimuthRadians));
