...
public class MainActivity extends AppCompatActivity
    ...
    private TextView uwbXyzInfo;
    private ImageView uwbAoaArrow;
    private TextView uwbRangingDevice;

    private static final byte PLATFORM_ANDROID = 0x02;
    private static final byte ERROR_UNSUPPORTED_PLATFORM = 0x01;
    private static final byte ERROR_APPLE_STACK_MISSING = 0x02;
    private static final byte ERROR_INVALID_CONFIG = 0x03;
    private static final byte ERROR_BUSY = 0x04;
    ...
    public enum MessageId {
        ...
        accessoryError((byte) 0x04),

        // Messages from the Uwb phone
        // Modified from 0xA5 to 0x0A to match iOS/Qorvo protocol
    ...
    private void initViews() {
        ...
        uwbXyzInfo = findViewById(R.id.uwb_xyz_info);
    ...
    public void processUwbRangingConfigurationData(byte[] data) {
        ...
        } else if (messageId == MessageId.accessoryError.getValue()) {
            handleAccessoryError(data);
        } else {
    ...
    public void startUwbRangingConfiguration() {
        bluetoothManagerImpl.transmit(new byte[]{MessageId.initialize.getValue(), PLATFORM_ANDROID});
    }
    ...
    private void displayRangingResult(RangingResult rangingResult) {
        ...
        if (rangingResult instanceof RangingResult.RangingResultPosition) {
            RangingSample sample = RangingSample.fromPosition((RangingResult.RangingResultPosition) rangingResult);
            Log.d(TAG, "Position sample distance=" + sample.getDistanceMeters()
                    + " azimuth=" + sample.getAzimuthDegrees()
                    + " elevation=" + sample.getElevationDegrees()
                    + " x=" + sample.getXMeters()
                    + " y=" + sample.getYMeters()
                    + " z=" + sample.getZMeters());
            updateRangingDistanceInfo(sample.getDistanceMeters());
            updateRangingAoaInfo(sample.getAzimuthDegrees(), sample.getElevationDegrees());
            updateRangingCartesianInfo(sample);
        } else if (rangingResult instanceof RangingResult.RangingResultPeerDisconnected) {
            Log.d(TAG, "Peer disconnected: address = " + rangingResult.getDevice().getAddress());
        } else {
            Log.d(TAG, "Received unknown rangingResult instance");
        }
    }
    ...
    private void handleAccessoryError(byte[] data) {
        if (data.length < 2) {
            Log.e(TAG, "Accessory error response too short");
            return;
        }
        ...
        String message;
        switch (errorCode) {
            case ERROR_UNSUPPORTED_PLATFORM:
                message = getString(R.string.uwb_error_unsupported_platform);
                break;
            case ERROR_APPLE_STACK_MISSING:
                message = getString(R.string.uwb_error_apple_stack_missing);
                break;
            case ERROR_INVALID_CONFIG:
                message = getString(R.string.uwb_error_invalid_config);
                break;
            case ERROR_BUSY:
                message = getString(R.string.uwb_error_busy);
                break;
            default:
                message = getString(R.string.uwb_error_unknown, errorCode & 0xFF);
                break;
        }

        Log.e(TAG, "Accessory error: " + message);
        runOnUiThread(() -> {
            uwbState.setText(getResources().getString(R.string.uwb_error));
    ...
    public void updateRangingAoaInfo(float azimuth, float elevation) {
        runOnUiThread(() -> {
            uwbAoaInfo.setText(getResources().getString(R.string.uwb_aoa_value, azimuth, elevation));
            uwbAoaArrow.setRotation(azimuth);
        });
    }

    public void updateRangingCartesianInfo(RangingSample sample) {
        runOnUiThread(() -> uwbXyzInfo.setText(getResources().getString(
                R.string.uwb_xyz_value,
                sample.getXMeters(),
                sample.getYMeters(),
                sample.getZMeters(),
    ...
    public void resetRangingInfo() {
        runOnUiThread(() -> {
            ...
            uwbXyzInfo.setText(getResources().getString(R.string.uwb_xyz_not_started));
